...
module.exports = async function newStudioSessionLeakModal(client, interaction){
  console.log(`Modal request Recieved`);



  const newStudioSessionLeakModal = new ModalBuilder()
  			.setCustomId('newStudioSessionLeakModal')
      ...
      // name of the song leaking
  		const leakName = new TextInputBuilder()
  			...
  			.setLabel("What's the new Studio Session Leak name?")
        ...
        const altLeakNames = new TextInputBuilder()
          ...
          const dateOfLeak = new TextInputBuilder()
            .setCustomId('dateOfLeak')
              // The label is the prompt the user sees for this input
            .setLabel("Input the date the Studio Session leaked")
              // Short means only a single line of text
      ...
      const fifthActionRow = new ActionRowBuilder().addComponents(notes);
  		...
  		newStudioSessionLeakModal.addComponents(firstActionRow, secondActionRow,thirdActionRow,fourthActionRow,fifthActionRow);
  console.log('Modal Configured and about to display');
  		// Show the modal to the user
  		await interaction.showModal(newStudioSessionLeakModal);
